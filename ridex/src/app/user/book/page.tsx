...
import { ArrowLeft, Bike, Car, Check, CheckCircle, ChevronRight, LocateFixed, MapPin, Navigation, Phone, Truck, TruckIcon } from 'lucide-react'
...
const VEHICLES = [
  ...
  {id: "truck", label: "Truck", Icon: TruckIcon, desc: "Heavy transport"},
]
    ...
    navigator.geolocation.getCurrentPosition(async ({coords}) => {
      try {
        ...
        if(data.features.length > 0) {
          ...
          const address = [p.housenumber, p.street, p.district, p.city, p.country, p.postcode].filter(Boolean).join(", ")
          setPickUp(address)
      ...
      >
        ...
        <div className='flex items-center gap-4 mb-6 px-1'>
          <motion.button
            whileTap={{scale: 0.88}}
            onClick={() => router.push("/")}
            className='w-11 h-11 rounded-xl bg-white border border-zinc-200 shadow-sm flex items-center justify-center hover:bg-zinc-50 transition-colors flex-shrink-0 cursor-pointer'
          >
                  ...
                  {VEHICLES.map((v, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        whileTap={{scale: 0.95}}
                        onClick={() => {
                          if(vehicle === v.id) setVehicle(undefined)
                            else setVehicle(v.id as vehicleType)
                        }}
                      ...
                      >
                        ...
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${active ? "bg-white" : "bg-zinc-200"}`}>
                          <v.Icon size={18} className={active ? "text-zinc-900" : "text-zinc-500"}/>
                        </div>
                    ...
                    <div className='relative z-30'>
                      <div className='flex items-center gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors'>
                        ...
                        onClick={useCurrentLocation}
                          whileTap = {{scale: 0.88}}
                          disabled={locating}
                          className='w-8 h-8 rounded-xl cursor-pointer bg-zinc-200 hover:bg-zinc-300 transition-colors flex items-center justify-center flex-shrink-0'
                        >
                          ...
                          >
                            ...
                            {pickUpSuggestions.map((p, i) => (
                              <motion.div
                                ...
                                className='flex items-center gap-3 w-full px-4 py-3 text-left hover:bg-zinc-50 transition-colors border-p border-zinc-100 last:border-0'
                              >
                    ...
                    <div className='relative z-10'>
                      <div className='flex items-center gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors'>
                        ...
                        <input 
                          ...
                          placeholder={pickUpCountry ? "Drop location" : "Enter pickup location first"}
                          className='flex-1 bg-transparent text-sm font-semibold text-zinc-900 placeholder:text-zinc-400 outline-none' 
                        />

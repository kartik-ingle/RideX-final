...
const STATUS_LABEL: Record<BookingStatus, {label: string; sublabel: string; dot: string}> = {
    ...
    awaiting_payment: {label: "Payment Pending", sublabel: "Waiting for the customer to complete the payment", dot: "bg-purple-400"},
    confirmed: {label: "Heading to Pickup", sublabel: "Booking is confirmed and waiting for the driver to start the ride", dot: "bg-amber-400"},
    started: {label: "Ride in progress", sublabel: "Ride has started and is currently ongoing", dot: "bg-emerald-400"},
    completed: {label: "Ride Completed", sublabel: "Ride has been completed successfully", dot: "bg-zinc-400"},
    cancelled: {label: "Ride Cancelled", sublabel: "Booking has been cancelled", dot: "bg-red-400"},
    rejected: {label: "Ride Rejected", sublabel: "Booking has been rejected", dot: "bg-red-400"},
    expired: {label: "Request Expired", sublabel: "Booking request has expired", dot: "bg-orange-400"}
}
...
function page() {
    ...
    const handleVerifyDropOtp = async () => {
        ...
        try {
            ...
            console.error("Failed to verify drop OTP", error)
            setLoadingDropOtp(false)
        ...
        <div className = "h-screen w-full bg-zinc-100 flex flex-col lg:flex-row overflow-hidden">
            <div className = "relative flex-1 h-full z-0">
                ...
                >
                    ...
                    <div className = "flex items-center gap-2 bg-white/95 backdrop-blur-sm px-4 py-3 rounded-full shadow-lg border border-zinc-100">
                        ...
                        <AnimatePresence mode = "wait">
                            {status === "confirmed" && !otpMode && !otpVerified && (
                                <motion.button
                                    ...
                                    animate = {{opacity: 1, y: 0}}
                                    exit = {{opacity: 0, y: 6}}
                                    className = "w-full cursor-pointer bg-zinc-900 hover:bg-zinc-800 active:scale-[0.97] text-white py-4 rounded-2xl font-bold text-sm tracking-wide transition-all flex items-center justify-center gap-2"
                                >
                            ...
                            {status === "confirmed" && otpMode && !otpVerified && (
                                ...
                                >
                                    ...
                                    <div className = "p-4 space-y-3">
                                        <p className = "text-xs text-zinc-500">Ask the customer for the 4 digit otp to start the ride.</p>
                                        ...
                                        <div className = "flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className = "flex-1 border bg-zinc-900 hover:bg-zinc-800 disabled:opacity-40 text-white py-2.5 rounded-xl text-sm font-semibold active:scale-[0.97] transition-all"
                                            >
                            ...
                            {status === "started" && dropOtpMode && (
                                ...
                                >
                                    ...
                                    <div className = "p-4 space-y-3">
                                        <p className = "text-xs text-zinc-500">Ask the customer for the 4 digit otp to complete the ride.</p>
                                        <div className = "flex justify-center">
                                            <input
                                                type = "text"
            ...
            {/* Mobile Panel */}
            <div className = "lg:hidden fixed bottom-0 left-0 right-0 z-20 pointer-event-none">
                <motion.div
                    animate = {{height: expanded ? "82vh" : 142}}
                    transition = {{type: "spring", stiffness: 320, damping: 38}}
                    className = "bg-white rounded-tl-3xl shadow-2xl pointer-events-auto overflow-hidden flex flex-col"
                >

...
function SearchPage() {
    ...
    const [pickUpLat, setPickUpLat] = useState(
      Number(params.get("pickuplat"))
    )

    const [pickUpLon, setPickUpLon] = useState(
      Number(params.get("pickuplon"))
    )

    const [dropLat, setDropLat] = useState(
      Number(params.get("droplat"))
    )

    const [dropLon, setDropLon] = useState(
      Number(params.get("droplon"))
    )
    ...
    useEffect(() => {
      ...
      router.replace(`/user/search?${url.toString()}`, {
        scroll: false,
      })
    }, [
      pickUp,
    ...
    <div className='min-h-screen bg-zinc-100 text-zinc-900 overflow-x-hidden'>
      ...
      <div className='relative w-full h-[52vh] z-0'>
        <SearchMap 
          pickUp={pickUp} 
          drop={drop} 
          onChange={(p, d, pLat, pLon, dLat, dLon) => {setPickUp(p); setDrop(d); setPickUpLat(pLat); setPickUpLon(pLon); setDropLat(dLat); setDropLon(dLon)}}
          onDistance = {setKm}
        />
      ...
      >
        ...
        <div className='px-5 lg:px-8 max-w-6xl mx-auto'>
          ...
          >
            ...
            <div>
              <h1 className="text-zinc-900 text-lg font-extrabold tracking-tight">
                {
                  loading 
                  ? 
                  "Finding vehicles"
          ...
          <AnimatePresence>
              {!loading && vehicles.length == 0 && (
                ...
                >
                  ...
                  <p className = "text-zinc-400 text-sm max-w-xs leading-relaxed">{meta.label || "Vehicles"} drivers are not available near your pickup right now.</p>

...
const pickUpIcon = new L.DivIcon({
    html: `<div style="display: flex; flex-direction: column; align-items:center; filter: drop-shadow(0px 4px 12px rgba(0,0,0,0.28)">
        <div style = "background: #0a0a0a; color: #fff; padding: 5px 13px; border-radius:100px; font-size:10px; font-weight: 800; letter-spacing: 0.1em; text-transform:uppercase; white-space:nowrap; font-family: system-ui">PICKUP</div>
...
const dropIcon = new L.DivIcon({
    html: `<div style="display: flex; flex-direction: column; align-items:center; filter: drop-shadow(0px 4px 12px rgba(0,0,0,0.28)">
        <div style = "background: #0a0a0a; color: #fff; padding: 5px 13px; border-radius:100px; font-size:10px; font-weight: 800; letter-spacing: 0.1em; text-transform:uppercase; white-space:nowrap; font-family: system-ui">DROP</div>
        <div style="width:2px; height:9px; background: #0a0a0a"></div>
        <div style = "width: 10px; height: 10px; background: #0a0a0a; border-radius: 50%; border:2.5px solid #fff; box-shadow:0 2px 6px rgba(0,0,0,0.3)" ></div>
    </div>`, 
    className: "",
...
const driverIcon = new L.DivIcon({
    html: `<div id = "car-marker" style = "
        ...
        transition: transform 0.3s cubic-bezier(0.34,1,1.56,0.64,1);
...
function LiveRideMap({driverLocation, pickUpLocation, dropLocation, mapStatus, onStats}: Props) {
        ...
        const fetchRoutes = async () => {
            try {
                if(mapStatus == "arriving") {
                    ...
                    console.log("pickup route", pickUpRoute)
                    console.log("drop route", dropRoute)
    ...
    return (
        <div className='relative h-full w-full bg-zinc-100'>
            <MapContainer zoom={12} center={pickUpLocation as any} zoomControl={false} style={{width: "100%", height: "100%"}}>
                <TileLayer
                    attribution='&copy; <a href="https://carto.com/">"CARTO"</a> contributors'
                    url="https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png"
                />

                {showPickMarker && <Marker position={pickUpLocation as any} icon={pickUpIcon} />}
                {dropLocation && <Marker position={dropLocation as any} icon={dropIcon} />}

                {driverLocation && <Marker position={driverLocation as any} icon={driverIcon} />}

                {showPickUpRoute && (
                    <Polyline positions={routeToPickUp} pathOptions={{color: "#888", weight: 4, lineCap: "round", dashArray: "2 10"}} />
                )}

...
type props={
    pickUp: string,
    drop: string,
    onChange: (p: string, d: string, pLat: number, pLon: number, dLat: number, dLon: number) => void,
...
const pickUpIcon = new L.DivIcon({
    html: `<div style="display: flex; flex-direction: column; align-items:center; filter: drop-shadow(0px 6px 18px rgba(0,0,0,0.22)">
...
const dropIcon = new L.DivIcon({
    html: `<div style="display: flex; flex-direction: column; align-items:center; filter: drop-shadow(0px 6px 18px rgba(0,0,0,0.22)">
        <div style = "background: #0a0a0a; color: #fff; padding: 5px 14px; border-radius:100px; font-size:10px; font-weight: 800; letter-spacing: 0.14em; text-transform:uppercase; white-space:nowrap; font-family: -apple-system, system-ui, sans-serif; box-shadow:0 2px 12px rgba(0,0,0,0.25);">DROP</div>
        <div style="width:2px; height:10px; background: #0a0a0a; opacity: 0.4"></div>
        <div style = "width: 13px; height: 13px; background: #0a0a0a; border-radius: 50%; border:3px solid #fff; box-shadow:0 0 0 2px rgba(0,0,0,0.15), 0 3px 10px rgba(0,0,0,0.3);" ></div>
    </div>`, 
    className: "",
...
function SearchMap({pickUp, drop, onChange, onDistance}: props) {
    ...
    const reverseGeoCoding = async (lat: number, lon: number) => {
            ...
            return [p.housenumber, p.street, p.district, p.city, p.country, p.postcode].filter(Boolean).join(", ")
    }
    ...
    const dragPickUp = async (lat: number, lon: number) => {
        ...
        onChange?.(addr ?? pickUp, drop, lat, lon, p2![0], p2![1])
    }
    ...
    const dragDrop = async (lat: number, lon: number) => {
        ...
        onChange?.(pickUp, addr ?? drop, p1![0], p1![1], lat, lon)
    }
  ...
  return (
    <div className='relative h-full w-full bg-zinc-100'>
        <MapContainer zoom={13} center={p1 ?? [0, 0]} zoomControl={false} style={{width: "100%", height: "100%"}}>
            <TileLayer
            ...
            {p1 && <Marker position={p1} icon={pickUpIcon} draggable
                eventHandlers={{
                    dragend:e => {
            ...
            />}
            {p2 && <Marker position={p2} icon={dropIcon} draggable 
                eventHandlers={{
                    dragend:e => {
                        const m = e.target.getLatLng()
                ...
                >
                    ...
                    <div className='relative w-14 h-14 flex items-center justify-center'>
                        ...
                        <motion.div 
                            ...
                            className='absolute inset-0 rounded-full border border-transparent border-t-zinc-300'
                        />
                        <MapPin size={15} className='text-zinc-800'/>
                    </div>

                    <div className='text-center'>
                        <p className='text-zinc-900 text-xs font-black tracking-[0.22em] uppercase'>Loading route...</p>
                        <p className='text-zinc-400 text-[10px] font-medium tracking-wider mt-0.5'>Please wait while we calculate the best path.</p>
                    </div>
        ...
        <AnimatePresence>
            {ready && km != null && (
                <motion.div
                    ...
                    className='absolute bottom-20 left-4 z-[500] flex items-center gap-2 border-zinc-200 px-3.5 py-2 rounded-xl shadow-lg'
                >

                    <Navigation2 size={13} className='text-zinc-900' />
                    <span className='text-zinc-900 text-xs font-bold'>{km} km</span>
                    <span className='w-px h-3 bg-zinc-200' />
                    <span className='text-zinc-900 text-xs font-bold'>~{Math.max(3, Math.round((km / 25) * 60))} min</span>

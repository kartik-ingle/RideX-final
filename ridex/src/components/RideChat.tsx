        ...
        <div className = "flex flex-col h-full min-h-0 bg-white rounded-2xl overflow-hidden border border-zinc-100">
            <div className = "flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-white border-b border-zinc-100">
                <div className = "relative flex-shrink-0">
                    <div className = "w-auto px-2 h-9 rounded-2xl bg-zinc-950 flex items-center justify-center text-white text-xs font-bold">
                        {otherName.split("")[0].toUpperCase()}
                    </div>
                    <span className = "absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-white" />
                </div>

                <div className = "flex-1 min-w-0">
                    <p className = "text-sm font-bold text-zinc-900 leading-none">{otherName.split(" ")[0]}</p>
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p className = "break-word text-md">{m.text}</p>
                        ...
                        <div className = "px-4 pt-3 pb-2">
                            <div className = "flex items-center justify-between mb-2">
                                ...
                                <button onClick = {() => setShowAI(false)}>
                                    <X className = " cursor-pointer text-zinc-400 hover:text-zinc-600" />
                                </button>
                            </div>
                            ...
                            {aiLoading? (
                                ...
                                <div className = "flex flex-col gap-1.5 max-h-[130px] overflow-y-auto pr-1">
                                    ...
                                    <button onClick = {() => getAISuggestions()} className = "text-[11px] text-violet-500 hover:text-violet-700 font-semibold text-center mt-1 transition-colors">
                                        Refresh Suggestions
                                    </button>
                                </div>
                            )}
                ...
                <div className = "flex items-center gap-2 bg-zinc-100 rounded-2xl pl-3 pr-1.5 py-1.5">
                    {messages.length > 0 && (
                        <motion.button
                            ...
                            className = {`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all ${showAI ? "bg-violet-600 text-white" : "bg-white text-violet-500 hover:bg-violet-50 border border-zinc-200"}`}
                        >
                            <Sparkles size = {14} />
                        </motion.button>
                    )}
                    ...
                    <motion.button
                        ...
                        className = {`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all
                            ${text.trim() ? "bg-zinc-950 text-white cursor-pointer hover:bg-zinc-800" : "bg-transparent text-zinc-300 cursor-not-allowed"}    
                        `}
                    >
                        <Send size = {14} />
